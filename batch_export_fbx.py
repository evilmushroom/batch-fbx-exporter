...
bl_info = {
    ...
    "version": (1, 0, 6),  # Version with Axis Label Fix
    "author": "Evilmushroom (Core Fixes by AI)",
...
def export_fbx(context, filepath, use_selection, bake_anim=False, bake_anim_use_all_actions=False):
    ...
    try:
        ...
        bpy.ops.export_scene.fbx(
            ...
            # *** USE AXIS SETTINGS FROM SCENE PROPERTIES ***
...
class ACTION_UL_list(UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            if hasattr(action, "export"):
                 row.prop(action, "export", text="")
            else:
            ...
            # Let Blender handle Fake User icon automatically
...
class OBJECT_UL_character_objects(UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        mesh_obj_item = item
        obj = mesh_obj_item.object
        row = layout.row(align=True)
        if obj:
             if obj.name in bpy.data.objects:
                 row.prop(mesh_obj_item, "export", text="")
                 row.prop(obj, "name", text="", emboss=False, icon_value=layout.icon(obj))
             else:
                 row.label(text=f"Missing: {obj.name}", icon='ERROR')
        else:
            row.label(text="Empty Slot", icon='QUESTION')
...
class ANIM_OT_push_actions_to_nla(Operator):
    ...
    def execute(self, context):
        ...
        pushed_count = 0
        actions_to_push = [action for action in bpy.data.actions if getattr(action, "export", False)]
...
class ANIM_OT_delete_selected_actions(Operator):
    ...
    @classmethod
    def poll(cls, context):
         return any(getattr(action, "export", False) for action in bpy.data.actions)
    def invoke(self, context, event):
        return context.window_manager.invoke_confirm(self, event)
    def execute(self, context):
        actions_to_remove = [action for action in bpy.data.actions if getattr(action, "export", False)]
        removed_count = 0
...
class OBJECT_OT_character_object_add(Operator):
    ...
    def execute(self, context):
        ...
        if active_obj:
             ...
             context.scene.character_object_index = len(context.scene.character_objects) - 1
             return {'FINISHED'}
...
class OBJECT_OT_character_object_remove(Operator):
    ...
    def poll(cls, context):
        scene = context.scene
        return scene.character_objects and 0 <= scene.character_object_index < len(scene.character_objects)
    def execute(self, context):
        scene = context.scene; index = scene.character_object_index
        if 0 <= index < len(scene.character_objects):
            scene.character_objects.remove(index)
            scene.character_object_index = min(max(0, index - 1), len(scene.character_objects) - 1)
            return {'FINISHED'}
...
class SCENE_OT_set_unreal_scale(Operator):
    ...
    bl_label = "Set Unit Scale to 0.01 (UE)"
    ...
    def execute(self, context):
        ...
        context.scene.unit_settings.system = 'METRIC'; context.scene.unit_settings.scale_length = 0.01
...
class OBJECT_OT_batch_export_fbx(Operator):
    ...
    def invoke(self, context, event):
        ...
        scene = context.scene
        export_path = bpy.path.abspath(scene.batch_export_path)
        existing_files = []
        ...
        armature = scene.character_armature
        character_name = scene.character_name if scene.character_name.strip() else "Character"
    ...
    def execute(self, context):
        # ... (Core export execution logic unchanged) ...
        # Overwrite confirmation handled by invoke calling execute only on OK/no conflict
            ...
            if scene.export_character:
                ...
                for item in scene.character_objects:
                    if item.object and item.export and item.object.name in scene.objects:
                        if item.object not in objects_to_export: objects_to_export.append(item.object)
                    elif item.object and item.export: self.report({'WARNING'}, f"Mesh '{item.object.name}' in list but not in scene. Skipping.")
                if len(objects_to_export) <= 1: self.report({'WARNING'}, "Exporting character but no meshes selected in list.")
                temp_action = None
                ...
                try: export_fbx(context, fbx_file, use_selection=True, bake_anim=False); self.report({'INFO'}, f"Exported character: {os.path.basename(fbx_file)}"); export_count += 1
                except Exception as e_char: self.report({'ERROR'}, f"Failed exporting character: {e_char}"); error_count += 1; print(traceback.format_exc())
...
class OBJECT_PT_batch_export_fbx_panel(Panel):
    ...
    def draw(self, context):
        ...
        if not is_correct_scale:
            ...
            row = warning_box.row()
            row.label(text=f"Current: {scene.unit_settings.scale_length:.4f}, Unreal requires: 0.01")
...
def update_select_all(self, context):
    for action in bpy.data.actions:
        setattr(action, "export", self.select_all_actions)
...
classes = (
    ...
    MeshObject,
    OBJECT_UL_character_objects,
    OBJECT_OT_character_object_add,
    OBJECT_OT_character_object_remove,
    OBJECT_OT_batch_export_fbx,
    OBJECT_PT_batch_export_fbx_panel,
    SCENE_OT_set_unreal_scale,
)
...
def register():
    ...
    bpy.types.Scene.select_all_actions = BoolProperty(name="Select All", description="Select or deselect all actions", default=True, update=update_select_all)
    ...
    # *** ADDED PROPERTIES for Axis and Toggle ***
    bpy.types.Scene.show_export_options = BoolProperty(
        name="Show Advanced Export Options",
        description="Show detailed FBX export settings like axis orientation",
        default=False # Start hidden
    )
    bpy.types.Scene.fbx_axis_forward = EnumProperty(
        name="Forward",
        description="Forward axis for FBX export",
        items=axis_items, # Use corrected items
        default='-Y' # Default for Unreal
    )
    bpy.types.Scene.fbx_axis_up = EnumProperty(
        name="Up",
        description="Up axis for FBX export",
        items=axis_items, # Use corrected items
        default='Z' # Default for Unreal
    )
...
def unregister():
    ...
    props_to_delete = [
        ...
        "embed_textures",
        # *** Added properties to delete ***
        "show_export_options", "fbx_axis_forward", "fbx_axis_up"
    ]
    for prop in props_to_delete:
        try: delattr(bpy.types.Scene, prop)
        except AttributeError: pass
    try: delattr(bpy.types.Action, "export")
    except AttributeError: pass
